...
# Contains the possibilities and rules considering the guesses
class World
  include Checks
  ...
  def initialize
    @all_colors = %w[R Y G U B W].shuffle
    @code_length = 4
  ...
  def process_results(rule)
    ...
    elsif (rule[:rc_rp] + rule[:rc_wp]).zero?
      @available_colors_collection.each_with_index do |available_colors, i|
        new = available_colors.reject { |color| rule[:guess].include?(color) }
        @available_colors_collection[i] = new
      end
    end
  end
